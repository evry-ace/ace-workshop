data "template_file" "ws" {
  ...
  vars = {
    ...
    aks_rbac_server_app_id           = var.aks_rbac_server_app_id
    aks_rbac_server_app_secret       = var.aks_rbac_server_app_secret
    aks_rbac_client_app_id           = var.aks_rbac_client_app_id
    aks_rbac_cluster_admins_group_id = data.azuread_group.ace_admins.id
    aks_rbac_cluster_users_group_id  = data.azuread_group.ace_users.id
  }
}
